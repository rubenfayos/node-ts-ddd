import { isFunction, isPlainObject, isString, isUndefined } from "@shared/common/utils/validation";
import { clc, yellow } from "./colors-util";
...
export class Logger implements ILogger {
  ...
  protected stringifyMessage(message: unknown, logLevel: LogLevel): string {
    if (isFunction(message)) {
      ...
      if (isClass) {
        return this.stringifyMessage((message as { name: string }).name, logLevel);
      }

      try {
        const result = (message as () => unknown)();
        return this.stringifyMessage(result, logLevel);
      } catch (err) {
        return this.colorize(`[Function threw error: ${(err as Error).message}]`, logLevel);
      }
    }
    ...
    return this.colorize(String(message), logLevel);
  }
  ...
  private isStackFormat(stack: any) {
    if (!isString(stack)) {
      return false;
    }
